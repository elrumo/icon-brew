...
export const actions = {
  ...
  async searchAlgolia(store, payload){
    ...
    // Save searchResult.hits into a .json file and name it algoliaIconbrew.json
    // store.dispatch('setDataToState', {state: 'jsonData', data: JSON.stringify(searchResult.hits)});
  ...
  async fetchIconCategories(store){
    ...
    // create a new function to fetch categories from Parse Server, use the class 'categories'
    ...
    queryResults.forEach((result) => {
      categories.push({
        ...
        noOfIcons: result.get('iconCount'),
    ...
    try {
      store.commit('setDataToState', {state: 'iconCategories', data: categories})
    } catch (error) {
  ...
  async addSuggestion(store, payload){
    let iconName = store.state.suggestedIconName + '/';
    let suggestedBy = store.state.suggestedIconUserName+'/';
    let email = store.state.suggestedIconEmail+'/';
    let notes = store.state.suggestedIconNotes.length == 0 ? '-/' : store.state.suggestedIconNotes+'/';
    let category = store.state.suggestedIconCategory.code;
    try {
      let postUrl = 'https://api.macosicons.com/api/icon-suggestions/create-suggestion/'+iconName+notes+suggestedBy+email+category;
      console.log("postUrl: ", postUrl);
      await axios.post(postUrl);

      await axios.post('https://api.macosicons.com/api/email/elrumo97@me.com/1/elias');
    } catch (error) {
...
export const getters = {
  ...
  getIconCategoriesForDropdown(state) {
    console.log(state.iconCategories);
  ...
  getIconSuggestionForm(state) {
    let form = {
      formData: {
        suggestedIconName: state.suggestedIconName,
        suggestedIconCategory: state.suggestedIconCategory,
        suggestedIconUserName: state.suggestedIconUserName,
        suggestedIconEmail: state.suggestedIconEmail,
        suggestedIconNotes: state.suggestedIconNotes,
      }
    }
    return form;
